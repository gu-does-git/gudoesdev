...
function compileMdx(_0) {
  return __async(this, arguments, function* (source, mdxOptions = {}, nextraOptions = {}, resourcePath = "") {
    ...
    try {
      const result = String(yield compiler.process(source)).replace("export const __nextra_title__", "const __nextra_title__").replace("export default MDXContent;", "");
...
var pagesDir = findPagesDir2(process.cwd()).pages;
...
export default MDXContent`.trimStart();
    ...
    const themeConfigImport = themeConfig ? `import __nextra_themeConfig__ from '${slash2(path5.resolve(themeConfig))}'` : "";
    const pageOpts = {
      filename,
      route: slash2(route),
      meta,
    ...
    const pageNextRoute = "/" + slash2(path5.relative(pagesDir, resourcePath)).replace(MARKDOWN_EXTENSION_REGEX, "").replace(/\/index$/, "").replace(/^index$/, "");
    return `
