...
var parseJsonFile = (content, path4) => {
  try {
    ...
    console.error(`[nextra] Error parsing ${path4}, make sure it's a valid JSON`, err);
    return {};
  }
};
...
function collectFiles(_0) {
  return __async(this, arguments, function* (dir, route = "/", fileMap = {}) {
    ...
    const items = (yield Promise.all(files.map((f) => __async(this, null, function* () {
      ...
      const fileRoute = (0, import_slash.default)(import_path3.default.join(route, name.replace(/^index$/, "")));
      if (f.isDirectory()) {
        if (fileRoute === "/api")
          return;
...
var NextraPlugin = class {
  ...
  apply(compiler) {
    compiler.hooks.beforeCompile.tapAsync("NextraPlugin", (_, callback) => __async(this, null, function* () {
      ...
      const result = yield collectFiles((0, import_find_pages_dir.findPagesDir)(process.cwd()).pages, "/");
